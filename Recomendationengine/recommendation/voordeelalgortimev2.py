...
def createrecomendeditemsrecords():
    ...
    cur.execute("select product._id, product.category_idcategory, product.gender_idgender, product.brand_idBrand,product.selling_price, product.discount from all_p WHERE product.discount IS NOT NULL and product.category_idcategory is not null order by category_idcategory, gender,discount asc")
    rows = cur.fetchall()
    ...
    return categorylst,genderlst,brandlst

def fillrecomendeditems(categorylst,genderlst):
    cur.execute("select product._id, product.category_idcategory, product.gender_idgender, product.brand_idBrand,product.selling_price, product.discount from all_p WHERE product.discount IS NOT NULL and product.category_idcategory is not null order by category_idcategory, gender,discount asc")
    rows = cur.fetchall()
    ...
    # in dit stukje code maak ik per category een selectie van 5 items per target/gender en schrijf dit weg naar de database
        for k in range(0, len(genderlst)):
            ...
            for row in rows:
                ...
                if count == 5:
                    break
                if (genderlst[k] in row):
                    # print(row)
                    count += 1
                    cur.execute(
                        "INSERT INTO recomendedpro (_ID, category, targetaudience, sellingprice, deal) VALUES ( %s, %s, %s, %s, %s)",
                        (row[0], row[1], row[2], row[3], row[4]))

    cur.execute("select distinct recomendedpro._id, recomendedpro.category, recomendedpro.targetaudience from recomendedpro order by recomendedpro._id asc")
    ...
    for prorow in prorows:
        cur.execute("INSERT INTO recomendedpro (_ID, category, targetaudience) VALUES ( %s, %s, %s)",
                    (prorow[0], prorow[1], prorow[2]))
    return
...
def getitemrecords(id):
    ...
    #edited last 2
    cur.execute("select product.id, product.category_idcategory, product.gender_idgender,product.brand_idBrand, product.price, all_p.discount from all_p where _id = '{}'".format(id))
    info = cur.fetchall()
    return info
